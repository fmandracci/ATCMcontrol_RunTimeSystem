...
app_expect_equal(const char *fn, char *line, unsigned ln, const char *cf)
{
    ...
    line += strspn(line, " \t");
    if (*line != '=') {
        fprintf(stderr, "[%s] %s: expecting `=' after identifier on line %d in file %s\n", __func__, fn, ln, cf);
...
int get_uint16(char *ptr, const char *id, uint16_t *value, unsigned line_num, const char *filename)
{
...
int get_uint32(char *ptr, const char *id, uint32_t *value, unsigned line_num, const char *filename)
{
    ptr = app_property_name_check(ptr, id);
    if (ptr == NULL) {
        return 0;
...
int get_name(char *ptr, const char *id, char *name, unsigned line_num, const char *filename)
{
    ...
    strncpy(name, ptr, MAX_NAMELEN - 1);
...
int get_system_conf(char *ptr, struct system_conf *system, unsigned line_num, const char *filename)
{
    ...
    r = get_uint16(ptr, "retries", &system->retries, line_num, filename);
    if (r) { return r; }
    r = get_uint16(ptr, "blacklist", &system->blacklist, line_num, filename);
    if (r) { return r; }
    r = get_uint16(ptr, "read_period_ms_1", &system->read_period_ms[0], line_num, filename);
    if (r) { return r; }
    r = get_uint16(ptr, "read_period_ms_2", &system->read_period_ms[1], line_num, filename);
    if (r) { return r; }
    r = get_uint16(ptr, "read_period_ms_3", &system->read_period_ms[2], line_num, filename);
    if (r) { return r; }
    r = get_name(ptr, "home_page", system->home_page, line_num, filename);
    if (r) { return r; }
    r = get_name(ptr, "start_page", system->start_page, line_num, filename);
    if (r) { return r; }
    r = get_uint16(ptr, "buzzer_touch", &system->buzzer_touch, line_num, filename);
    if (r) { return r; }
    r = get_uint16(ptr, "buzzer_alarm", &system->buzzer_alarm, line_num, filename);
    if (r) { return r; }
    r = get_uint16(ptr, "pwd_timeout_s", &system->pwd_timeout_s, line_num, filename);
    if (r) { return r; }
    r = get_name(ptr, "pwd_logout_page", system->pwd_logout_page, line_num, filename);
    if (r) { return r; }
    r = get_uint16(ptr, "screen_saver_s",&system->screen_saver_s, line_num, filename);
    if (r) { return r; }
    r = get_uint16(ptr, "slow_log_period_s", &system->slow_log_period_s, line_num, filename);
    if (r) { return r; }
    r = get_uint16(ptr, "fast_log_period_s", &system->fast_log_period_s, line_num, filename);
    if (r) { return r; }
    r = get_uint16(ptr, "max_log_space_MB", &system->max_log_space_MB, line_num, filename);
    if (r) { return r; }
    r = get_uint16(ptr, "trace_window_s", &system->trace_window_s, line_num, filename);
    if (r) { return r; }
...
int get_serial_conf(char *ptr, struct serial_conf *serial, unsigned line_num, const char *filename)
{
    ...
    r = get_uint32(ptr, "baudrate", &serial->baudrate, line_num, filename);
    if (r) { return r; }
    r = get_uint16(ptr, "databits", &serial->databits, line_num, filename);
    if (r) { return r; }
    r = get_char(ptr, "parity", &serial->parity, line_num, filename);
    if (r) { return r; }
    r = get_uint16(ptr, "stopbits", &serial->stopbits, line_num, filename);
    if (r) { return r; }
    r = get_uint16(ptr, "silence_ms", &serial->silence_ms, line_num, filename);
    if (r) { return r; }
    r = get_uint16(ptr, "timeout_ms", &serial->timeout_ms, line_num, filename);
    if (r) { return r; }
    r = get_uint16(ptr, "max_block_size", &serial->max_block_size, line_num, filename);
    if (r) { return r; }
    if (*ptr != '\0') {
...
int get_tcp_ip_conf(char *ptr, struct tcp_ip_conf *tcp_ip, unsigned line_num, const char *filename)
{
    ...
    r = get_uint16(ptr, "silence_ms", &tcp_ip->silence_ms, line_num, filename);
    if (r) { return r; }
    r = get_uint16(ptr, "timeout_ms", &tcp_ip->timeout_ms, line_num, filename);
    if (r) { return r; }
    r = get_uint16(ptr, "max_block_size", &tcp_ip->max_block_size, line_num, filename);
    if (r) { return r; }
    if (*ptr != '\0') {
...
int get_canopen_conf(char *ptr, struct canopen_conf *canopen, unsigned line_num, const char *filename)
{
    ...
    r = get_uint32(ptr, "baudrate", &canopen->baudrate, line_num, filename);
    if (r) { return r; }
    r = get_uint16(ptr, "max_block_size", &canopen->max_block_size, line_num, filename);
    if (r) { return r; }
    if (*ptr != '\0') {
...
int app_config_load(struct system_ini *system_ini)
{
    ...
    fprintf(stderr, "[%s]: loading '%s' ...", __func__, APP_CONFIG_FILE);
    cf = fopen(APP_CONFIG_FILE, "r");
    if (cf == NULL) {
        fprintf(stderr, "[%s] error opening %s: %s\n", __func__, APP_CONFIG_FILE, strerror(errno));
        return 1;
    }
