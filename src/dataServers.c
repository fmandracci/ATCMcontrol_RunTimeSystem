...
static inline void changeServerStatus(uint32_t s, enum ServerStatus status);

static void startServerTiming(uint32_t s);
static inline void updateServerTiming(uint32_t s);
static inline void updateServerIdleTime(uint32_t s);
static inline void updateServerBusyTime(uint32_t s);
...
void *serverThread(void *arg)
{
    ...
    // thread init
    xx_pthread_setsched(SCHED_FIFO, XX_PRIO_MEDIUM); // serverThread
    xx_pthread_mutex_init(&theServer->mutex);
    ...
    case RTU_SRV: {
        ...
        snprintf(device, MAXPATHLEN, SERIAL_DEVNAME, SERIAL_DEVNUM(theServer->u.serial.port));
        ...
        if (theServer->ctx) {
            modbus_rtu_set_serial_mode(theServer->ctx, MODBUS_RTU_RS485);
            modbus_rtu_set_rts(theServer->ctx, MODBUS_RTU_RTS_DOWN); // i.e. nRTS
            // modbus_rtu_set_rts_delay(theServer->ctx, 3);
        }
    ...
    while (engineStatus != enExiting) {
        ...
        if (server_socket == -1) {
            ...
            case RTU_SRV:
                if (modbus_connect(theServer->ctx)) {
                    ...
                    modbus_set_response_timeout(theServer->ctx, timeout_tv.tv_sec, timeout_tv.tv_usec);
                }
                break;
            case TCP_SRV:
        ...
        if (server_socket < 0) {
            XX_GPIO_SRV_CLR(s);
            xx_sleep_ms(THE_SERVER_DELAY_ms);
            XX_GPIO_SRV_SET(s);
            continue;
        }
        ...
        if (theServer->protocol == RTU_SRV) {
            ...
            XX_GPIO_SRV_CLR(s);
            xx_sleep_ms(theServer->silence_ms);
            XX_GPIO_SRV_SET(s);
        } else {
        ...
        case TCPRTU_SRV:
            ...
            for (master_socket = 0; master_socket <= fdmax; ++master_socket) {
                ...
                if (master_socket == server_socket) {
                    ...
                    if (newfd == -1) {
#ifdef VERBOSE_DEBUG
                        fprintf(stderr, "[%s]: server accept() error: %s\n", __func__, strerror(errno));
#endif
                    } else {
                        FD_SET(newfd, &refset);
...
#ifdef VERBOSE_DEBUG
                        fprintf(stderr, "[%s]: New connection from %s:%d on socket %d\n",
                            __func__, inet_ntoa(clientaddr.sin_addr), clientaddr.sin_port, newfd);
#endif
                    }
                } else {
...
#ifdef VERBOSE_DEBUG
    fprintf(stderr, "[%s]: %s: status = %d\n", __func__, theServers[s].name, status);
#endif
}
