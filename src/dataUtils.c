...
{
#if 1 //#ifdef VERBOSE_DEBUG
    fprintf(stderr, "[%s]: %d --> %d\n", __func__, engineStatus, status);
#endif
    engineStatus = status;
...
unsigned doWriteVariable(unsigned addr, unsigned value, uint32_t *values, uint32_t *flags, unsigned addrMax)
{
    ...
    if (CrossTable[addr].Protocol == PLC) {
        ...
        if (CrossTable[addr].Types == vt_BIT || CrossTable[addr].Types == vt_BYTE_BIT
            || CrossTable[addr].Types == vt_WORD_BIT || CrossTable[addr].Types == vt_DWORD_BIT ) {
        ...
        uint16_t d = CrossTable[addr].device;
        ...
        if (d >= theDevicesNumber) {
            ...
            if (values) {
                ...
                for (n = 1; (addr + n) < (base + size) && (addr + n) <= addrMax; ++n)
                {
                    ...
                    // in Modbus clients we cannot mix vt_BIT and non vt_BIT variables
                    if ((protocol == RTU || protocol == TCP || protocol == TCPRTU || protocol == RTU_SRV || protocol == TCP_SRV || protocol == TCPRTU_SRV)
                        && ((type == vt_BIT && CrossTable[addr + n].Types != vt_BIT)
                           || (type != vt_BIT && CrossTable[addr + n].Types == vt_BIT))) {
                        break;
                    }
                    if (flags) {
                        ...
                        if (flags[addr + n] == 0
                        && !((type == vt_BYTE_BIT || type == vt_WORD_BIT || type == vt_DWORD_BIT)
                    ...
                    if (flags && flags[addr + n] == 0) {
                        ...
                        // normal case
                        if (CrossTable[addr + n].Types == vt_BIT || CrossTable[addr + n].Types == vt_BYTE_BIT
                         || CrossTable[addr + n].Types == vt_WORD_BIT || CrossTable[addr + n].Types == vt_DWORD_BIT ) {
                            // the engine uses only the bit#0 for bool variables
                            if (values[addr + n] & 1) {
                                request.Values[n] = 1;
                            } else {
...
void writeQdataRegisters(uint16_t addr, uint32_t value, uint8_t status)
{
    ...
    if (addr == PLC_WATCHDOGEN) {
        ...
        register int n = addr - PLC_FastIO_1;

        if (xx_gpio_is_enabled(n) && xx_gpio_is_output(n)) {
            if (value) {
                xx_gpio_set(n);
            } else {
                xx_gpio_clr(n);
    ...
    if (xx_retentives_ptr && addr > 0 && addr <= LAST_RETENTIVE && status != DATA_WARNING) {
        xx_retentives_ptr[addr -1] = value;
