...
static void fieldbusReset(uint16_t d);
static enum fieldbusError fieldbusRead(uint16_t d, uint16_t DataAddr, uint32_t DataValue[], uint16_t DataNumber);
static enum fieldbusError fieldbusWrite(uint16_t d, uint16_t DataAddr, uint32_t DataValue[], uint16_t DataNumber);

static void doWriteDeviceRetentives(uint32_t d);

static inline void changeDeviceStatus(uint32_t d, enum DeviceStatus status);
static inline void changeNodeStatus(uint32_t d, uint16_t node, enum NodeStatus status);

static unsigned checkAddr(unsigned d, unsigned DataAddr, unsigned DataNumber, int writing);

static void startDeviceTiming(uint32_t d);
static inline void updateDeviceTiming(uint32_t d);
static inline void updateDeviceIdleTime(uint32_t d);
static inline void updateDeviceBusyTime(uint32_t d);

static uint16_t modbusRegistersNumber(uint16_t DataAddr, uint16_t DataNumber);

static inline unsigned get_byte_bit(uint8_t data, unsigned n);
static inline unsigned get_word_bit(uint16_t data, unsigned n);
static inline unsigned get_dword_bit(uint32_t data, unsigned n);
//static inline void set_byte_bit(uint8_t *data, unsigned n, unsigned value);
static inline void set_word_bit(uint16_t *data, unsigned n, unsigned value);
static inline void set_dword_bit(uint32_t *data, unsigned n, unsigned value);
...
void *clientThread(void *arg)
{
    ...
    uint16_t read_addr[MAX_PRIORITY];      // current read address in crosstable for each priority
    RTIME read_time_ns[MAX_PRIORITY];   // next read time for each priority

    // data for each fieldbus operation
    uint16_t readOperation; // read/write
    uint16_t DataAddr = 0;  // variable address in the crosstable
    uint32_t DataValue[MAX_VALUES]; // max 64 reads and 16 writes
    uint32_t DataNumber;    // max 64 reads and 16 writes
    uint16_t Data_node;     // global index of the node in theNodes[]
    ...
    case MECT:
    case RTU_SRV:
        xx_pthread_setsched(SCHED_FIFO, XX_PRIO_MEDIUM); // clientThread (no TCP)
        break;
    case TCP:
    case TCPRTU:
    case TCP_SRV:
    case TCPRTU_SRV:
        xx_pthread_setsched(SCHED_OTHER, XX_PRIO_MEDIUM); // clientThread (TCP)
        break;
    default:
        ;
    }
    ...
    case RTU: {
        ...
        snprintf(device, MAXPATHLEN, SERIAL_DEVNAME, SERIAL_DEVNUM(theDevice->u.serial.port));
        ...
        if (theDevice->modbus_ctx) {
            modbus_rtu_set_serial_mode(theDevice->modbus_ctx, MODBUS_RTU_RS485);
            modbus_rtu_set_rts(theDevice->modbus_ctx, MODBUS_RTU_RTS_DOWN); // i.e. nRTS
            // modbus_rtu_set_rts_delay(theDevice->modbus_ctx, 3);
            // https://libmodbus.org/reference/modbus_set_error_recovery
            modbus_set_error_recovery(theDevice->modbus_ctx, MODBUS_ERROR_RECOVERY_PROTOCOL); // MODBUS_ERROR_RECOVERY_NONE MODBUS_ERROR_RECOVERY_LINK
        }
    }   break;
    case TCP: {
    ...
    case TCPRTU:
        ...
        break;
    case CANOPEN: {
        uint8_t channel = theDevice->u.can.bus;
    ...
    while (engineStatus != enExiting) {
        ...
        if ((engineStatus != enRunning) || theDevice->status == NO_HOPE) {
            XX_GPIO_DEV_CLR(d);
            xx_sleep_ms(THE_CONNECTION_DELAY_ms);
            XX_GPIO_DEV_SET(d);
            continue;
        }
        ...
        if (DataAddr == 0) {
            int rc;
            RTIME next_ns;
            int some_enabled_node = FALSE;
            int n;

            // the thread must wake up anyway
            next_ns = theDevice->current_time_ns + THE_MAX_CLIENT_SLEEP_ns;

            // check if there is at least some enabled node
            for (n = 0; n < theNodesNumber; ++n) {
                if (theNodes[n].device == d && theNodes[n].status != NODE_DISABLED) {
                    some_enabled_node = TRUE;
                    break;
                }
            }
            if (some_enabled_node) {
                // search for next programmed read
                for (prio = 0; prio < MAX_PRIORITY; ++prio) {
                    if (we_have_variables[prio]) {
            ...
            if (next_ns > theDevice->current_time_ns) {
                ...
                do {
                    ...
                    rc = sem_clockwait(&newOperations[d], CLOCK_MONOTONIC, &abstime);
                    if (rc)
                        saved_errno = errno;
                    else
                        saved_errno = 0;
...
#ifdef VERBOSE_DEBUG
                    invalid_timeout = invalid_permission = other_error = FALSE;
#endif
                    errno = saved_errno;
...
#error unknown size
                ...
                if (timeout && ! some_enabled_node) {
#ifdef VERBOSE_DEBUG
                    fprintf(stderr, "[%s]: %s@%09llu ns: timeout with no enabled nodes (next_ns=%09llu ns)\n", __func__, theDevice->name, theDevice->current_time_ns, next_ns);
#endif
                    continue;
                } else {
                    ...
                    updateDeviceTiming(d);
#ifdef VERBOSE_DEBUG
                    fprintf(stderr, "[%s]: %s@%09llu ns: timeout=%d, some_enabled nodes=%d (next_ns=%09llu ns)\n", __func__, theDevice->name, theDevice->current_time_ns, timeout, some_enabled_node, next_ns);
#endif
#ifdef VERBOSE_DEBUG
                    if (invalid_timeout || invalid_permission || other_error) {
                        fprintf(stderr, "[%s]: %s@%09llu ns: woke up because %s (%09llu ns = %ld s + %ld ns)\n",
                            __func__,
                            theDevice->name,
                            theDevice->current_time_ns,
                            timeout?"timeout":(invalid_timeout?"invalid_timeout":(invalid_permission?"invalid_permission":(other_error?"other_error":"signal"))),
                            next_ns, abstime.tv_sec, abstime.tv_nsec);
                    }
#endif
                }
            } else {
        ...
        if (DataAddr == 0) {
            ...
            pthread_mutex_lock(&theCrosstableClientMutex);
            {
                ...
                if (DataAddr == 0) {
                    uint16_t addr = 0;
                    int at_least_one_node = FALSE;

                    for (int n = 0; n < theNodesNumber; ++n) {
                        if (theNodes[n].device == d && theNodes[n].status != NODE_DISABLED) {
    ...
    #ifdef VERBOSE_DEBUG
                                fprintf(stderr, "%s@%09llu ns: found=%d addr=%u \n", theDevice->name, theDevice->current_time_ns, found, addr);
    #endif
                                if (found && checkAddr(d, addr, CrossTable[addr].BlockSize, FALSE) > 0) {
    ...
    #ifdef VERBOSE_DEBUG
                                    fprintf(stderr, "%s@%09llu ns: read %uPSF [%u] (was [%u]), will check [%u]\n", theDevice->name, theDevice->current_time_ns, prio+1, DataAddr, addr, read_addr[prio]);
    #endif
                                    break;
                                } else {
                                    if (found)
                                    {
                                        fprintf(stderr, "wrong addr?\n");
                                    }
                                    ...
                                    read_addr[prio] = 0;
    #ifdef VERBOSE_DEBUG
                                    fprintf(stderr, "%s@%09llu ns: no read %uHPSF will restart at %09llu ns\n", theDevice->name, theDevice->current_time_ns, prio+1, read_time_ns[prio]);
    #endif
                                }
                            }
                        }
        ...
        case NOT_CONNECTED:
            ...
            case RTU:
                ...
                if (modbus_connect(theDevice->modbus_ctx) >= 0) {
                    modbus_set_response_timeout(theDevice->modbus_ctx, response_timeout.tv_sec, response_timeout.tv_usec);
            ...
            case MECT: // connect()
                theDevice->mect_fd = xx_mect_connect(
        ...
        case MECT:
            if (theDevice->silence_ms > 0) {
                XX_GPIO_DEV_CLR(d);
                xx_sleep_ms(theDevice->silence_ms);
                XX_GPIO_DEV_SET(d);
            }
            break;
        default:
    ...
    case CANOPEN:
        ...
        break;
    case MECT: // close()
        xx_mect_close(theDevice->mect_fd);
        break;
    case RTU_SRV:
    case TCP_SRV:
    case TCPRTU_SRV:
...
static void fieldbusReset(uint16_t d)
{
    ...
    case MECT: // fieldbusReset()
        xx_mect_close(theDevices[d].mect_fd);
        break;
    case RTU_SRV:
    case TCP_SRV:
    case TCPRTU_SRV:
...
static enum fieldbusError fieldbusRead(uint16_t d, uint16_t DataAddr, uint32_t DataValue[], uint16_t DataNumber)
{
    ...
    uint16_t i, r, regs, device, server, x;
    int e = 0;
    ...
    case TCPRTU_SRV:
        ...
        case TCP:
        case TCPRTU:
            if (CrossTable[DataAddr].Types == vt_BIT) {
        ...
        case TCPRTU_SRV:
            ...
            if (device != 0xffff) {
                ...
                if (server != 0xffff && theServers[server].mb_mapping != NULL) {
                    ...
                    {
                        register uint32_t base = CrossTable[DataAddr].Offset;
                        bzero(uintRegs, sizeof(uintRegs));
                        for (r = 0; r < regs; ++r) {
...
#define DO_READ_OUTPUTS
                ...
                if (CrossTable[DataAddr + i].Output) {
                    ...
                    case vt_DWORD_BIT:
                        ...
                        do {
                            ...
                            && ++i);
                        r += (CrossTable[DataAddr + i].Types == vt_DWORD_BIT) ? 2 : 1;
                        break;
                    case vt_UINT8:
                    case vt_UINT16:
                    case vt_INT16:
...
#endif
                    register uint8_t a = (uintRegs[r] & 0x00FF);
                    register uint8_t b = (uintRegs[r] & 0xFF00) >> 8;
                    register uint8_t c = (uintRegs[r + 1] & 0x00FF);
                    register uint8_t d = (uintRegs[r + 1] & 0xFF00) >> 8;
                    ...
                    case vt_DWORD_BIT: {
                        ...
                        register uint32_t offset = CrossTable[DataAddr + i].Offset;
    ...
    case CANOPEN:
        device = CrossTable[DataAddr].device;
        if (device != 0xffff) {
            uint8_t channel = theDevices[device].port;
            for (i = 0; i < DataNumber; ++i) {
                register enum varTypes vartype = CrossTable[DataAddr + i].Types;
                register uint32_t offset = CrossTable[DataAddr + i].Offset;
    ...
    case MECT: // fieldbusRead()
        for (i = 0; i < DataNumber; ++i) {
            ...
            if (CrossTable[DataAddr + i].Types == vt_REAL) {
                float value;
                e = xx_mect_read_ascii(theDevices[d].mect_fd, CrossTable[DataAddr + i].NodeId, CrossTable[DataAddr + i].Offset, &value);
                memcpy(&DataValue[i], &value, sizeof(uint32_t));
                if (verbose_print_enabled) {
                    fprintf(stderr, "[%s]: %s, %s=%f err=%d\n", __func__, theDevices[d].name, CrossTable[DataAddr + i].Tag, value, e);
                }
            } else if (CrossTable[DataAddr + i].Types == vt_UINT16) {
                unsigned value = 0;
                e = xx_mect_read_hexad(theDevices[d].mect_fd, CrossTable[DataAddr + i].NodeId, CrossTable[DataAddr + i].Offset, &value);
                DataValue[i] = value;
                if (verbose_print_enabled) {
...
static enum fieldbusError fieldbusWrite(uint16_t d, uint16_t DataAddr, uint32_t DataValue[], uint16_t DataNumber)
{
    ...
    case TCPRTU_SRV:
        ...
        if (CrossTable[DataAddr].Types == vt_BIT) {
            ...
            for (i = 0, r = 0; i < DataNumber && r < regs; ++i) {
                ...
                case vt_DWORD_BIT: {
                    ...
                    register uint16_t base = CrossTable[DataAddr + i].BlockBase;
                    register uint16_t size = CrossTable[DataAddr + i].BlockSize;
                    register uint32_t offset = CrossTable[DataAddr + i].Offset;
                    register enum varTypes vartype = CrossTable[DataAddr + i].Types;
        ...
        for (i = 0, r = 0; i < DataNumber && r < regs; ++i) {
            register uint8_t a = (DataValue[i] & 0x000000FF);
            register uint8_t b = (DataValue[i] & 0x0000FF00) >>  8;
            register uint8_t c = (DataValue[i] & 0x00FF0000) >> 16;
            register uint8_t d = (DataValue[i] & 0xFF000000) >> 24;
        ...
        case TCPRTU_SRV:
            ...
            if (device != 0xffff) {
                ...
                if (server != 0xffff && theServers[server].mb_mapping != NULL) {
                    pthread_mutex_lock(&theServers[server].mutex);
                    {
                        register uint16_t base = CrossTable[DataAddr].Offset;
                        for (r = 0; r < regs; ++r) {
                            if ((base + r) < REG_SRV_NUMBER) {
        ...
        if (device != 0xffff) {
            ...
            for (i = 0; i < DataNumber; ++i) {
                ...
                case vt_DWORD_BIT:  {
                    ...
                    register uint32_t offset = CrossTable[DataAddr + i].Offset;
                    register enum varTypes vartype = CrossTable[DataAddr + i].Types;
                    uint32_t buffer = 0x00000000; // BYTE_BIT, WORD_BIT, DWORD_BIT
                    ...
                    switch (vartype) {
                    case vt_BYTE_BIT: {
                        register uint8_t a = (uint8_t)(buffer & 0x000000FF);
                        e = CANopenWritePDOByte(channel, offset, a);
                    }   break;
                    case vt_WORD_BIT: {
                        register uint16_t a = (uint16_t)(buffer & 0x0000FFFF);
    ...
    case MECT: // fieldbusWrite()
        for (i = 0; i < DataNumber; ++i) {
            ...
            if (CrossTable[DataAddr + i].Types == vt_REAL) {
                float value;
                memcpy(&value, &DataValue[i], sizeof(uint32_t));
                e = xx_mect_write_ascii(theDevices[d].mect_fd, CrossTable[DataAddr + i].NodeId, CrossTable[DataAddr + i].Offset, value);
            } else if (CrossTable[DataAddr + i].Types == vt_UINT16) {
                unsigned value = DataValue[i];
                e = xx_mect_write_hexad(theDevices[d].mect_fd, CrossTable[DataAddr + i].NodeId, CrossTable[DataAddr + i].Offset, value);
            }
            if (e == -1) {
                retval = CommError;
                break;
...
static uint16_t modbusRegistersNumber(uint16_t DataAddr, uint16_t DataNumber)
{
    ...
    for (i = 0; i < DataNumber; ++i) {
        switch (CrossTable[DataAddr + i].Types) {
        case  vt_DINT: case  vt_DINTDCBA: case  vt_DINTCDAB: case  vt_DINTBADC:
        case vt_UDINT: case vt_UDINTDCBA: case vt_UDINTCDAB: case vt_UDINTBADC:
        case vt_REAL: case vt_REALDCBA: case vt_REALCDAB: case vt_REALBADC:
            retval += 2;
            break;
        case vt_UINT8:
        case  vt_INT16: case  vt_INT16BA:
        case vt_UINT16: case vt_UINT16BA:
            retval += 1;
            break;
        case vt_BIT:
            retval += 1;
        ...
        case vt_DWORD_BIT: {
            ...
            do {
                ...
                && CrossTable[DataAddr + (i + 1)].Offset == offset
                && ++i);
            retval += (vartype == vt_DWORD_BIT) ? 2 : 1;
        }   break;
        default:
            ; // FIXME: assert
        }
